   ...
   // success!
    ...
    if(!preg_match($regex, $_POST['email'])){
	 ...
	 if(!empty(array_filter($_FILES['image']['name']))){
		...
		$file_name = $_FILES['image']['name']['$key']['$val'];
...
if($_POST['fname']!='' && $_POST["lname"]!='' && $_POST["email"]!='' && $_POST['conform']&& $_POST["pwd"]!='' && $_POST["uname"]!='' && $_POST["gender"] !='' && $_POST["conform"] == $_POST["pwd"] && $_FILES !=''){
		  $sql ="INSERT INTO anu (fname, lname, email, pwd,  uname, gender,image)VALUES('".$_POST['fname']."', '".$_POST['lname']."', '".$_POST['email']."','".trim(md5($_POST['pwd']))."' , '".$_POST['uname']."','".$_POST['gender']."','".$_FILES['image']['name']['$key']."' )";
...
<html>
	...
	Uname<input type="text" name="uname" value="<?php if(isset($_POST['uname'])) echo $_POST['uname'];?>"><span style="color:red;"> <?php if(isset($unameErr)) echo $unameErr ;?></span><br><br>
	 ...
	 <input type="radio" name="gender" value="female">female <br><br>
	  <input type="file" name="image[]" multiple id="image"><span style="color:red;"><?php if(isset($imgerr)) echo $imgerr;?></span><br><br>
